...
makeSuite('Rex Governance V2 tests', deployGovernance, (testEnv: TestEnv) => {
  ...
  before(async () => {
    const {gov, executor, strategy, rex, users, minter} = testEnv;
    ...
    flashAttacks = await deployFlashAttacks(rex.address, minter.address, gov.address);
    await rex.connect(minter.signer).approve(flashAttacks.address, MAX_UINT_AMOUNT);
    ...
    await waitForTx(await rex.connect(user5.signer).delegate(user2.address));
    ...
    const balanceAfter = await rex.connect(user1.signer).balanceOf(user1.address);
    // Pending => Active
    // => go tto start block
  ...
  describe('Testing voting functions', function () {
    ...
    it('Vote a proposal without quorum => proposal failed', async () => {
      ...
      const balance = await rex.connect(user1.signer).balanceOf(user1.address);
    ...
    it('Vote a proposal with quorum => proposal succeeded', async () => {
      ...
      const balance1 = await rex.connect(user1.signer).balanceOf(user1.address);
      await expect(gov.connect(user1.signer).submitVote(proposal2Id, true))
        .to.emit(gov, 'VoteEmitted')
    ...
    it('Vote a proposal with quorum via delegation => proposal succeeded', async () => {
      ...
      const balance2 = await rex.connect(user1.signer).balanceOf(user2.address);
      const balance5 = await rex.connect(user2.signer).balanceOf(user5.address);
    ...
    it('Vote a proposal with quorum but not vote dif => proposal failed', async () => {
      ...
      const balance4 = await rex.connect(user4.signer).balanceOf(user4.address);
    ...
    it('Vote a proposal with quorum and vote dif => proposal succeeded', async () => {
      ...
      const balance3 = await rex.connect(user3.signer).balanceOf(user3.address);
    ...
    it('Vote a proposal by permit', async () => {
      ...
      const balance = await rex.connect(minter.signer).balanceOf(user3.address);
  ...
  describe('Testing create function', function () {
    ...
    it('should not create a proposal when proposer has not enought power', async () => {
      ...
      // Give not enough REX for proposition tokens
      await setBalance(user, minimumCreatePower.sub('1'), testEnv);
    ...
    it('should create proposal when enough power via delegation', async () => {
      ...
      await waitForTx(await rex.connect(user2.signer).delegate(user.address));
    ...
    it('should not create a proposal without targets', async () => {
      ...
      // Give enought REX for proposition tokens
      await setBalance(user, minimumCreatePower, testEnv);
  ...
  describe('Testing queue duplicate actions', function () {
    beforeEach(async () => {
      ...
      const {gov, executor, rex, users} = testEnv;
